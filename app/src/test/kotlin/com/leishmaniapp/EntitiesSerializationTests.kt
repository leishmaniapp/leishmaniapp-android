...
import com.leishmaniapp.services.serialization.UUIDSerializer
...
class EntitiesSerializationTests {
    ...
    fun diagnosticElementNameByStringShouldReturnDiseaseDiagnosticElementName() {
        ...
        // Deserialize
        Assert.assertEquals(
            diagnosticElement.id,
            Json.decodeFromString<DiagnosticElement>(serializedValue).id
        )
    }
    ...
    fun diagnosisJsonSerializationIncludesAllEntities() {
        ...
        val images = List(2) { iterator ->
            MockGenerator.mockImage(
                sample = iterator,
                size = 1944,
                processed = ImageAnalysisStage.Analyzed
            ).copy(elements = setOf())
        }.toMutableList()
        ...
        images[0] = images[0].copy(
            elements = setOf(
                SpecialistDiagnosticElement(
                    id = LeishmaniasisGiemsaDisease.elements.first(),
                    amount = 10
                ),
        ...
        val diagnosis = com.leishmaniapp.domain.entities.Diagnosis(
    ...
    fun payloadJsonTesting() {
        ...
        val imageProcessingPayload =
            image.toProcessingPayload(UUID.randomUUID(), MockDotsDisease, "bucket", "key")
